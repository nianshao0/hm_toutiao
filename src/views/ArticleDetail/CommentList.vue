<template>
  <div>
    <!-- 评论列表 -->
    <!-- 1. van-list最好在这个div内，让底部文字/加载中，作为此div内容，容器padding-bottom底部位置也挤上去了-看得见 -->
    <div class="cmt-list" :class="{ 'art-cmt-container-1': isShowCmtBox, 'art-cmt-container-2': !isShowCmtBox }">
      <!-- 2. van-list包裹列表，组件内源码，检测网页滚动事件，判断滚动位置是否快到达内容高度(触底) -> 触发load事件执行
         -> loading: 为true -> 底部加载文字出现 -> 内部认为选择处于加载中状态 -->
      <!-- 3. immediate-check 作用: 让list组件不要上来就检查是否触底
          原因: list中列表内容是异步网络请求回来的，标签挂载时，没有高度，list就会判断触底执行一次onLoad方法
          问题: created发一次，onLoad立刻又发一次，请求2此第一页的数据/上来就请求第一页和第二页数据 -->
      <van-list v-model="loading" :finished="finished" finished-text="没有更多评论了，请发表评论" @load="onLoad"
...
<script>
import { commentsListAPI, commentDisLikingAPI, commentLikingAPI, commentSendAPI } from '@/api/index.js'
...
export default {
  data() {
    return {
      ...
      loading: false,
      finished: false,
      lastId: null // 下一页(偏移id), 一定初始值null(第一页不需要传, axios遇到null忽略此参数)
    }
  },
  async created() {
    const res = await commentsListAPI({
      id: this.$route.query.art_id // 文章 id
    })
    console.log(res)
  ...
  methods: {
    ...
    // 评论按钮 -> 点击事件 -> 把第一条评论滑到最上面
    commentClickFn() {
      // 看扩展里ppt，只要设置window.scrollTo(0,文章内容高度)
      // JS代码是在html+css运行后，真实DOM已经在网页上了，从document往下取标签是ok的
      // const articleHeight = document.querySelector('.article-container').scrollHeight
      // // window.scrpllTo() 使网页进行滚动，设置响应的坐标，就可以让网页滚动到屏幕的最顶端
      // // 如果底下没有内容了，则不再滚动
      ...
      // 为何选择like-box不选择第一条评论，业务头部导航会挡住
    ...
    blurFn() {
      ...
      // 原因：高的评论容器-在页面点击发布的一瞬间，输入框失去了焦点，被v-if和v-else移除了整个标签
      // 导致整个事件没来得及执行
      // 解决：失去焦点时，变量值，在最后执行
      setTimeout(() => {
