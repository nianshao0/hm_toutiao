...
// 2. 在里面用bottomText的值，判断用户点击是否"返回"
...
export default {
  props: {
    ...
    isShow: {
      type: Boolean,
      default: true // 例如首页的地方不想再去动了，首页肯定没传isShow值进来，所以首页需要显示
    }
  },
  data() {
    return {
  ...
  methods: {
    formatTime: timeAgo, // 函数体是tiemAgo
    // 反馈面板-选项选择事件
    onSelect(action, item) {
      ...
      if (action.name === '反馈垃圾内容') {
        ...
        this.show = false // 关闭-下面的cancel会帮你把反馈选项切换来
      }
    },
    // 反馈面板-底部按钮-点击事件
    cancelFn() {
